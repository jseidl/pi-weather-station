...
class PMS5003Sensor(SensorBase):
    ...
    def get_measurements(self):
        ...
        if not self.pms:
            self.logger.error("PMS5003 not available")
            return {}
        ...
        self.logger.debug("Reading PMS5003")
        pms_data = self.pms.read()
        self.logger.debug(f"PMS5003 read. Data: {pms_data}")
        ...
        for psize in psizes:
            ...
            regular = pms_data.pm_ug_per_m3(psize)
            atmos = pms_data.pm_ug_per_m3(psize, True)

            key_prefix = f"pm{psize}".replace('.', '_')

            measurements[key_prefix] = regular
            measurements[f"{key_prefix}_atmos"] = atmos
